...
function mergeDescending(left, right) {
    ...
    useEffect(() => {
        const fetchUserCredentials = async () => {
          try {
            ...
            const response = await axios.get('https://quiz-app-iazp.onrender.com/api/get-user-whole-details', {
              headers: {
                Authorization: `Bearer ${$FSA_auth_token}`,
              },
            });
            console.log(response.data);
            setFirstOne(response.data.username);
  ...
  <div className={styles.firstPlace}>
      ...
      />
      <p className={styles.name}>🥇 {firstOne}</p>
